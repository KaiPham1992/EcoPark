...
open class ContainerViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public private(set) lazy var containerViewTapGesture: UITapGestureRecognizer = {
        let gesture = UITapGestureRecognizer(
            target: self,
            action: #selector(ContainerViewController.didtapContainerView(_:))
        )
        gesture.delegate = self
        return gesture
    }()
    ...
    public func setDrawerStateNoAnimation(_ state: DrawerState) {
...
extension ContainerViewController: MenuProtocol {
    ...
    func pushViewController(presentingController: UIViewController) {
        if rootNavigation?.visibleViewController == presentingController {
            print("the same view controller")
            self.setDrawerState(.closed)
            return
        }
        ...
        rootNavigation?.isNavigationBarHidden = false
//        self.setDrawerState(.closed)
        self.setDrawerStateNoAnimation(.closed)
    }
    
    func presentViewController(presentingController: UIViewController) {
      
        let navigationVC = UINavigationController(rootViewController: presentingController)
//        rootNavigation?.popToRootViewController(animated: false)
        
        rootNavigation?.dismiss()
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            self.rootNavigation?.present(controller: navigationVC)
        }
...
//        self.setDrawerState(.closed)
    ...
    {
        return  rootNavigation?.topViewController ?? UIViewController()
    }
    
    func getNV() -> Any {
        return rootNavigation ?? nil
    }
}
